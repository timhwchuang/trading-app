# AGENTS.md — theman 專案 AI Agent 協作守則
...
| 工具 | 自動載入設定 | 驗證方式 |
| ---- | ------------ | -------- |
| **Cursor** | [`.cursor/rules/theman-safety-guardrails.mdc`](.cursor/rules/theman-safety-guardrails.mdc)（`alwaysApply: true`）+ [`.cursor/rules/theman-agents-compliance.mdc`](.cursor/rules/theman-agents-compliance.mdc) | 開新 Agent 對話後 rules 應出現在專案規則中 |
...
- **`TODO.md`**（根目錄）
  - 更新「目前狀態」表格（Phase 0~7 勾選與說明）。
  - 在對應 Phase 區塊補上實作細節、驗收條件、變更理由。
  - 歷史 Code Review 對照永遠以本檔為準（行號已過時）。
...
- **`docs/` 整個資料夾**（索引見 [`docs/README.md`](docs/README.md)），尤其是：
  ...
  - `BackTesting.md`、`AuditContract.md`、`UATReminder.md`、`BeforePilot.md`、`WindowsOps.md`、`CALLBACK_GUARDRAILS.md`。
  - `CodeReview#*.md` 是歷史紀錄，新 review 結論要反應到 `TODO.md` 與 `WeeklyStatus.md`。
...
| 條件 | 參考 |
| ---- | ---- |
| `python run_tests.py` 全綠（目前 **139** 項） | 本檔 §10 |
| 永豐**模擬** API；`simulation: true` | `UATReminder.md` |
| `TICK_ARCHIVE=1`（hard gate）；建議 `KBARS_ARCHIVE=1` | `UATReminder.md`、`WeeklyStatus.md` |
| Phase 0~2 驗收項有 log 證據 + 人工對帳 | `UATReminder.md`、`TODO.md` Phase 3 |
...
├── scripts/windows/        # Windows 排程註冊等運維腳本
├── src/                    # 唯一真實原始碼（src-layout）
│   ├── runtime/            # ★ 核心執行宿主
│   │   └── engine.py       # TradingEngine（Live 與回測共用狀態機）
│   ├── live/               # Live 入口（python -m live）
│   ├── backtest/           # 回測引擎
│   │   ├── engine.py       # BacktestEngine（注入 strategy + MockBroker + VirtualClock）
│   │   ├── mock_broker.py  # 撮合、滑價、成交模擬
│   │   └── replay.py       # tick 回放（支援 .csv / .csv.gz）
...
目前 repo **尚無 CI**（無 `.github/workflows`）。Agent 在改動時應維持以下標準，並在合適時建議人類補齊自動化。
...
3. **程式內**：日虧上限停新進場、session flatten、斷線停新進場——見 engine 與 `UATReminder.md` 驗收項。

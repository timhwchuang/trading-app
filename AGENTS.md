...
| 條件 | 參考 |
| ---- | ---- |
| `python run_tests.py` 全綠（trading-app 整合層 **~69** 項；siblings 各自有獨立測試） | 本檔 §10 |
...
CI 骨架已落地：[`.github/workflows/ci.yml`](.github/workflows/ci.yml)（push/PR 至 `main` 或 `feat/*`/`fix/*` 跑 `python run_tests.py` on ubuntu + py3.11）。**本地 merge 前仍須全綠**（trading-app ~69 項）。若 GitHub Actions 未啟用，以本地 `run_tests.py` 為準。

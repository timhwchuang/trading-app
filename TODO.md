...
| **Phase 6 策略真實化** | **骨架已落地（旗標預設全關）**：P6-1～P6-3 + **P6-6 ✅**；**Live 前須 UAT tick 回測校準 k 再開旗標**；P6-4/5 待做 — 見 [`docs/CodeReview#2.md`](docs/CodeReview%232.md) |
...
- [x] **P2-8 測試可注入 mock API**
  - 已落地：`TradingEngine(api=...)`、`test_helpers.make_strategy()`、`test_deal_state_machine.py`
...
- [x] **P6-1 大週期趨勢濾網（Trend Filter）** — ✅ **骨架已落地（`trend_filter_enabled: false`）**；🔴 **校準後才開（Live hard gate）**
  - 已落地：`strategy_phase6.py`（`compute_trend`：5m/15m 降採樣 + `ema` / `vwap_slope`）；`config.yaml` 參數齊全；entry audit `trend_dir` / `trend_strength`
  - 待 UAT：以 tick 回測校準 `trend_timeframe_min` / `ema_period` / `vwap_slope_min`；比較濾網開關的秒停損率
...
→ ✅ P6-6 生存指標（已落地）
→ 🟡 P6-1 / P6-2 / P6-3 骨架已落地 → **UAT 回測校準 k 後開旗標**（Live hard gate）
...
- [ ] Widen Strategy Protocol / decouple host from VWAP-specific API (`.momentum`, `reset_momentum`, `manage_exit`, audit builders) so non-VWAP strategies survive a tick.
- [ ] Align `reset()` vs `reset_momentum()` host call sites with the Protocol.
- Next: more example strategies, richer MarketContext if needed, possible dynamic plugin loading.

See `src/strategy/base.py`. Host = `runtime.TradingEngine` / `backtest.BacktestEngine`; decision logic = `strategy.*` plugins. **Only VWAP runs end-to-end today.**

...
| **Phase 6 策略真實化** | **骨架已落地（旗標預設全關）**：P6-1～P6-3 + **P6-6 ✅**；**P6-1-CAL 校準工作流進行中**（見下方；**非 UAT Blocker**）；**Live 前須 UAT tick 校準後才開旗標**；P6-4/5 待做 — 見 [`docs/CodeReview#2.md`](docs/CodeReview%232.md) + 雙 Code Review (2026-06-16) |
...
單元測試：`tests/test_*`（`python run_tests.py` **146** 項；mock API，見 P2-8 `test_helpers.make_host`）
...
- [x] **P6-1-CAL-3 param_sweep 納入趨勢參數** — ✅（`5ac2959` + CQR LGTM）
  - sweep grid 接受 `trend_filter_enabled` + `trend_min_strength`（既有 generic apply 機制）
  - 報表 row 自動附加 `veto_metrics`（含 veto_rate/delta）；舊 grid 行為 100% 不變
  - 測試驗證 trend grid 4 筆 row 均有 key；155 tests OK
  - 檔案：`src/sweep/param_sweep.py`、`tests/sweep/test_param_sweep.py`
  - CQR review：直接 merge（結構正確、安全、無破壞、為 CAL-5 pipeline 打基石）。
...
→ ✅ P6-6 生存指標（已落地）
→ 🟡 P6-1 骨架已落地 → **P6-1-CAL 校準工作流**（A 類 1→5 金鑰前；B 類 6→8 UAT 後）→ 人類核可才開旗標
...
| 2026-06-16 | **Follow-up code review fixes** (per unmerged branch issues list)：P0-1 exchange_time.py `Any` import；P0-2/3 CAL-3 sweep key casing (snake→UPPER via tolerant apply + SWEEP_FIELD_TO_CONST + live _config getattr in engine refresh_atr so patches take effect)；P1-4/6 veto_metrics wiring (harvest SIGNAL_AUDIT from _AuditCaptureHandler in param_sweep _run, separate veto/allowed, feed real captured instead of always-[] )；P2-7 remove dead `default_window` param. All 155 tests green; snake grid keys now correctly affect TREND_* at runtime. P2-1 remains explicit stub (consistent with commit msg + TODO). |

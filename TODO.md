...
| **Phase 7 策略介面**   | **✅ 已落地**：`Strategy` Protocol + `BaseStrategy`（`trading-engine`）；預設 plugin=`strategy-vwap-momentum`；host=`TradingEngine` / `BacktestEngine.host` — 見 [`docs/Architecture.md`](docs/Architecture.md) |
| **Phase 8 三 repo 拆分**   | **✅ 已發布**：`trading-engine@v0.2.0`、`trading-backtest@v0.1.1`、`strategy-vwap-momentum@v0.1.1`、`trading-app@v0.1.1`；`trading_app_engine_ports()` 顯式接線；re-export shim 已移除。🔜 事件層（NDJSON sink）待第一段乾淨 UAT 後；`session.sync_positions` Action 字串化 — 見 [`docs/Architecture.md`](docs/Architecture.md) |
...
測試：`python run_tests.py` — **trading-app 整合層 69 項**（mock API，見 P2-8 `test_helpers.make_host`）；kernel 測試在 sibling packages（backtest 27、strategy 31、engine 各自 `run_tests.py`）
...
- [x] **P2-7 秒停損率量化（UAT / Pilot 硬指標）**
  ...
  - 用法：`cd src; python -m reporting C:\logs\trading-app-uat.log`（`--json` / `--quick-sl-sec 5`）
...
- [x] **P4-4 進程守護（Windows）**
  - 已落地：[`docs/WindowsOps.md`](docs/WindowsOps.md)、`scripts/windows/start-trading-app.ps1`、`register-task.ps1`（預設任務 `trading-app-vwap`）
...
- [x] **P6-1-CAL-5 BackTestingSpec 校準 SOP** — ✅（本節 + 同步）
  ...
  - 檔案：`docs/BackTestingSpec.md`（新章節 + 狀態表）、`TODO.md`、`docs/WeeklyStatus.md`
...
| `LOG_FILE`                | （空） | UAT 建議設 `C:\logs\trading-app-uat.log`                      |
...
| 2026-06-16 | **trading-app v0.1.1**（等 UAT KEY 期間）：sibling bugfix（backtest `str` close、strategy `trend_dir`）；移除 `theman_*` aliases；Windows ops / docs 同步；三 repo tag `v0.1.1` 已 push。trading-app 69 tests 全綠。 |

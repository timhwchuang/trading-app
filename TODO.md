...
單元測試：`tests/test_*`（`python run_tests.py` **139** 項；mock API，見 P2-8 `test_helpers.make_host`）
...
- [x] **P6-1 大週期趨勢濾網（Trend Filter）** — ✅ **骨架已落地（`trend_filter_enabled: false`）**；🔴 **校準後才開（Live hard gate）**
  - 已落地：`strategy/trend.py`（`compute_trend` + `min_strength` Level 2 強化：只有 HTF 波段夠強才標 Long/Short）；`config.yaml` 參數齊全；entry audit `trend_dir` / `trend_strength`
  - 待 UAT：以 tick 回測校準 `trend_timeframe_min` / `ema_period` / `trend_slope_min` / `trend_min_strength`；比較濾網開關 + 不同 strength 閾值的秒停損率與勝率
...
| `trend_min_strength`     | P6-1 L2| 最小強度（Level 2）：未達此 |strength| 則降為 Flat；實戰關鍵參數 |

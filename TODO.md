...
- [x] Adapt `VWAPMomentumStrategy` (renamed from VWAPMomentumLogic for clarity) as the first (and current default) implementation.
- [x] Update `strategy/__init__.py` (now truly exports the pluggable story), test_helpers, and add injection tests.
...
See the Strategy interface in `src/strategy/base.py` and usage in engine constructors. This makes "engine is engine, backtest is backtest, strategies are plugins".
